...
test.use({
  ...
  launchOptions: {
    args: [
      ...
      `--use-file-for-fake-video-capture=${config.lms.faceFixtures.multiFace}`,
    ],
  },
});

const TC_BY_TITLE = {
  "TC-5 Leg B: Suspicious activity - multiple faces detected warning": { sheet: "Student", tcId: 5 },
};

test.describe("Student | Proctoring Pro | TC-5 Leg B", () => {
  test.afterEach(async ({}, testInfo) => {
    ...
    try {
      const out = getWriter().write({
        ...
        tcId: meta.tcId,
        status,
        detail: `[Leg B: Multi Face] ` + detailParts.join("\n"),
      });
  ...
  test("TC-5 Leg B: Suspicious activity - multiple faces detected warning", async ({
    actions,
  }) => {
