...
test.use({
  ...
  launchOptions: {
    args: [
      "--start-maximized",
      // Specifically omit --use-fake-device-for-media-stream so there is no fake camera device registered
    ],
  },
});

const TC_BY_TITLE = {
  "TC-4 No camera device available surfaces a camera-not-detected error": { sheet: "Student", tcId: 4 },
};

test.describe("Student | Proctoring Pro | TC-4", () => {
  ...
  test("TC-4 No camera device available surfaces a camera-not-detected error", async ({
    actions,
  }) => {
